...
This could be coded as a single transformation series:

> Tail: (0), absent; (1), present, red, scaly, straight; (2), present, red, scaly, curly;
> (3), present, red, hairy, straight, [....] (8), present, blue, hairy, curly

and a Sankoff matrix constructed:
...
costs <- matrix(c(
  ...
  c(rep('.', 5), 0, rep('.', 2)),
  c(rep('.', 6), 0, rep('.', 1)),
  c(3, 3, 2, 2, dots, 0)) , byrow=TRUE, ncol=length(state))
colnames(costs) <-  state
rownames(costs) <- c("(0), absent", 
                     "(1), present, red, scaly, straight",
                     "(2) present, red, scaly, curly",
                     "(3) present, red, hairy, straight",
                     dots,
                     "(8), present, blue, hairy, curly")
knitr::kable(costs, capiton="Sankoff matrix")
```
```

Note that entries denoted `3*` could instead be coded with a cost of 1, if the loss
of a tail was to be treated separately from the gain of a tail

```{R Sankoff is no good, echo=FALSE, fig.height=5, fig.width=8}
