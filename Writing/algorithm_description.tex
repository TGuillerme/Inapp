...
\begin{enumerate}
    ...
    \item \label{1stUp:exit} \textbf{If} any of the node's descendants are not tips, move to the next node and \textit{go} to \ref{1stUp:enter}. \textbf{If} both descendants are tips, move to the closest non-visited node and \textit{go} to \ref{1stUp:enter}. Once all nodes have been visited, end the first uppass.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \label{2ndUp:ancestorA1} \textbf{If} the node's states are the same as its ancestor's, \textit{go} to \ref{2ndUp:exit}; \textbf{else}, \textit{go} to \ref{2ndUp:ancestorA2}.
