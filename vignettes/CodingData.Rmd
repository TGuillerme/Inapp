...
One way to emphasize this distinction in character matrices is to reserve the `0` token to denote absence, using higher integers to refer to states of transformational characters:
...
vignettePlot(better, col, na=FALSE, legend.pos = 'topleft')
mtext("Tail, glow-in-the-dark pigment: (-), inapplicable; (0), absent; (1), present",
      side=1, line=0, outer=TRUE)
vignettePlot(worse, col, na=FALSE, legend.pos = 'topleft')
...
<mark>We discussed in an [earlier chapter](FailedSolutions.html) that, if we only 
...
- This has a side-effect that we'll study with an invariant character</mark>
