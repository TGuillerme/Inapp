...
## "Parsimony uninformative" characters can inform parsimony (!)
...
vignettePlot(tailTree, col)
vignettePlot(bodyTree, col)
mtext("Tail colour", outer=TRUE)

vignettePlot(tailTree, bcol, FALSE)
vignettePlot(bodyTree, bcol, FALSE)
mtext("Body colour", outer=TRUE)
```

In trees in which the tail has a single origin, blue colouration also evolves once -- even though So the tree on the left, which reconstructs a single origin of the tail, a single origin of a blue tail colouration, and two changes in body colour, is preferred to the tree on the right, which reconstructs a single change in body colour but two origins of the tail, and two origins of blue colouration on the tail.
