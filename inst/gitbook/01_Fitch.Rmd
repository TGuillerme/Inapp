...
redBlue <- colours <- c(TreeTools::brewer[[2]], 'grey')
...
setMargins()
plot(tree, adj = 0.5)
tiplabels(char, cex = 1, bg = TreeTools::brewer[[2]][char + 1], adj = 1)
...
The uppass traversal employs the following rules [@Fitch1971]:
 ...
 2. If there is _at least one state in common_ between both left and right direct descendants of the current node, resolve the node as being _the states in common_ between its ancestor and both its descendants.
 3. If there is _no state in common_ between its descendants, resolve the node as being _the states in common_ between the ancestor and the current node.
...
edge_col <- TreeTools::brewer[[2]][c(2,2,1,  2,1,1,2,
                                     2,2,2,  1,1,1,2)]
...
#edge_col <- TreeTools::brewer[[2]][3-c(2,2,1,1,  1,1,2,
#                                 2,2,2,1,  1,1,2)]
#
#graphics::plot(tree, show.tip.label = TRUE, main='AccTran',
#               type = "phylogram", use.edge.length = TRUE, cex = 1, adj = 0,
...
edge_col <- TreeTools::brewer[[2]][c(1,1,1,rep(2, 7), 1, 2, 2, 1)]
...
edge_col <- TreeTools::brewer[[2]][c(1,1,1,2, 2, 2,1,2, 2, 2, 2, 2, 2, 2, 1, 2, 2, 1)]
...
char <- c(0,0,'?', 1, 0, 1)
edge_col <- TreeTools::brewer[[2]][c(1,1,1,2,2, 2, 2, 2, 1, 2)]
