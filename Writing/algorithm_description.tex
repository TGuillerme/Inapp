...
% if the any tip has an ambiguous character -A, solve using its descendant as the union of descendant and tip
\begin{enumerate}
    ...
    \item \label{1stUp:exit} \textbf{If} any of the node's descendant is not a tip, move to the next node and \textit{go} to \ref{1stUp:enter}. \textbf{Else} \textbf{if} one of its descendant

    if the tip has both an inapplicable and applicable state
        if the current node is inapplicable, solve the tip as inapplicable only
        else remove the inapplicable state from the tip


    \textbf{If} both descendants are tips, move to the closest non-visited node and \textit{go} to \ref{1stUp:enter}. Once all nodes have been visited, end the first uppass.
\end{enumerate}
...
\begin{enumerate}
    Visit each tip on the tree, if the tip only contains an inapplicable token, set its tracker to NA, else, if it contains only Appli tokens, set it to appli.
    Else if the tip is ambiguous (A-NA), then set it's tracker to it's ancestor's (first uppass) applicability.

    \item  Enter on any cherry on the tree and move to its most recent common ancestor; \textbf{then}, \textit{go} to \ref{2ndDp:enter}.

    If either trackers of the descendants contains an applicable token, set this node's tracker to applicable. Else set it to inapplicable.

    \label{2ndDp:nodeAppli} If the node had an applicable state in the previous pass (first up), go to \ref{2ndDp:enter}; else leave the node state unchanged and go to \ref{2ndDp:exit}.
    ...
    \item \label{2ndDp:CountRegion} \textbf{If} both of the node's descendants' trackers are applicable, \textbf{\textit{increment}} the tree length (applicable region increment) \textbf{then} \textit{go} to \ref{2ndDp:exit}; \textbf{else} just \textit{go} to \ref{2ndDp:exit}.
...
\begin{enumerate}
    ...
    \item \label{2ndUp:CountRegion} \textbf{If} both of the node's descendants' trackers are applicable, \textbf{\textit{increment}} the tree length (applicable region increment) \textbf{then} \textit{go} to \ref{2ndDp:exit}; \textbf{else} just \textit{go} to \ref{2ndUp:exit}.
