...
```{r EGdownpass, echo = FALSE, fig.width = 6, fig.height = 6}
...
The downpass is quiet simple and follow the following rules for the two possible cases in the traversal:
...
```{r plotDownpass, fig.width = 6, fig.height = 6}
...
For example, it seems clear that the sate of node `n14` could not be `0` since both it's ancestor and one of its descendant as an ambiguous state of `01`.
...
```{r plotUppass, fig.width = 6, fig.height = 6}
## Plotting the first downpass
